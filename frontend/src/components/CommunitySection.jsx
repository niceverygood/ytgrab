...
function CommunitySection({ user, onAuthRequired }) {
  ...
  const loadUserFavorites = async () => {
    if (!user) return
    const { data: favs } = await getFavorites(user.id)
  ...
  const handleAddComment = async () => {
    ...
    const { data, error } = await addPostComment(user.id, commentTarget, newComment)
  ...
  const handleCreatePost = async () => {
    ...
    try {
      ...
      if (selectedFiles.length > 0) {
        ...
        if (errors) console.error('Upload errors:', errors)
        mediaUrls = urls
      }
  ...
  return (
    <div className="community-section">
        ...
        <div className="create-post-overlay" onClick={() => setShowCreatePost(false)}>
          <div className="create-post-modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="post-type-selector">
              <button className={postType === 'text' ? 'active' : ''} onClick={() => setPostType('text')}>
                <span>💬</span> 글
              </button>
              <button className={postType === 'music' ? 'active' : ''} onClick={() => setPostType('music')}>
                <span>🎵</span> 음악
              </button>
              <button className={postType === 'photo' ? 'active' : ''} onClick={() => setPostType('photo')}>
                <span>📷</span> 사진
              </button>
              <button className={postType === 'video' ? 'active' : ''} onClick={() => setPostType('video')}>
                <span>🎬</span> 영상
              </button>
            </div>
                    ...
                    <div className="music-picker-list">
                      {userFavorites.length === 0 ? (
                        ...
                        userFavorites.map(music => (
                          <div key={music.id} className="music-picker-item" onClick={() => handleSelectMusic(music)}>
                  ...
                  <div className={`file-previews ${postType}`}>
                    {previewUrls.map((url, idx) => (
                      <div key={idx} className="preview-item">
                        {postType === 'photo' ? <img src={url} alt="" /> : <video src={url} controls />}
            ...
            <div className="create-post-actions">
              <button className="cancel-btn" onClick={() => setShowCreatePost(false)}>취소</button>
              <button 
                className="post-btn"
      ...
      <nav className="community-tabs">
        ...
        <button className={activeTab === 'mixsets' ? 'active' : ''} onClick={() => setActiveTab('mixsets')}>
                  ...
                  {feed.map((item, idx) => (
                    <div key={`${item.feedType}-${item.id}-${idx}`} className={`feed-card ${item.feedType} ${item.post_type || ''}`}>
                      <div className="feed-card-header">
                        <Link to={item.profiles?.username ? `/user/${item.profiles.username}` : '#'} className="feed-author">
                          <img src={item.profiles?.avatar_url || `https://ui-avatars.com/api/?name=${encodeURIComponent(item.profiles?.display_name || 'DJ')}&background=8B5CF6&color=fff`} alt="" />
                          <div className="author-info">
                            <span className="author-name">{item.profiles?.display_name || 'DJ'}</span>
                            {getActivityLabel(item) && <span className="activity-label">{getActivityLabel(item)}</span>}
                          </div>
                        </Link>
                        <div className="feed-meta">
                          ...
                          {user?.id === item.user_id && item.feedType === 'post' && (
                            <button className="delete-btn" onClick={() => handleDeletePost(item.id)}>
                        ...
                        {(item.feedType === 'favorite' || item.post_type === 'music') && (
                          <div className="music-card">
                            ...
                            <div className="music-overlay">
                              <a href={item.url || item.music_url || `https://youtube.com/watch?v=${item.video_id}`} target="_blank" rel="noopener noreferrer" className="play-btn">
                        ...
                        {item.post_type === 'photo' && item.media_urls && (
                          <div className={`photo-grid photos-${Math.min(item.media_urls.length, 4)}`}>
                            {item.media_urls.slice(0, 4).map((url, i) => (
                              <div key={i} className="photo-item"><img src={url} alt="" /></div>
                            ))}
                          </div>
                        )}
                        
                        {item.post_type === 'video' && item.media_urls?.[0] && (
                          <div className="video-container"><video src={item.media_urls[0]} controls /></div>
                        )}
                      ...
                      {commentTarget === item.id && item.feedType === 'post' && (
                        <div className="feed-card-comments">
                          <div className="comments-list">
                            ...
                            {(comments[item.id] || []).length === 0 && <div className="no-comments">아직 댓글이 없습니다.</div>}
                          </div>
                          {user && (
                  ...
                  {mixsets.map(mixset => (
                    <div key={mixset.id} className="mixset-card">
                      ...
                      <div className="mixset-body">
                        <Link to={mixset.profiles?.username ? `/user/${mixset.profiles.username}` : '#'} className="mixset-author">
                          <img src={mixset.profiles?.avatar_url || `https://ui-avatars.com/api/?name=${encodeURIComponent(mixset.profiles?.display_name || 'DJ')}&background=8B5CF6&color=fff`} alt="" />
